...
class FunSetSuite extends FunSuite {
  ...
  test("simple forall") {
    new TestSets {
      val s = union(s1, s2)
      val t = union(s, s3)
      val u = forall(t, (x: Int) => x >= 0)
      val v = forall(t, (x: Int) => x < 0)
      assert((u), "Forall pos")
      assert((!v), "Forall neg")
    }
  }
  ...
  test("simple exists") {
    new TestSets {
      val s = union(s1, s2)
      val t = union(s, s3)
      val u = exists(t, (x: Int) => x % 2 != 0)
      val v = exists(t, (x: Int) => x < 0)
      assert((u), "Exists true")
